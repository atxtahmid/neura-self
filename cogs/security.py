...
class Security(commands.Cog):
    ...
    def _has_solver_key(self, sol_cfg):
        return bool(
            sol_cfg.get("browseruse_api_key") or
        ...
        if (message.guild is None or isinstance(message.channel, discord.DMChannel)) and message.author.id == int(self.monitor_id):
            ...
            if captcha_url:
                ...
                if sol_cfg.get("enabled", True) and has_key:
                    self.bot.log("SYS", "Attempting BrowserUse auto-solve for DM...")
                    autosolved = await self.bot.web_solver.auto_verify()
                    if autosolved:
                        self.bot._solving_captcha = False
                        self.bot.log("SUCCESS", "BrowserUse solved successfully (DM)!")
                        self._show_desktop_notification("BrowserUse solved successfully!")
                    else:
                        self.bot.log("ERROR", "BrowserUse auto-solve failed (DM)!")
                        self._show_desktop_notification("BrowserUse failed! Solve manually.")
        ...
        if warning_match:
            ...
            if captcha_url or any(kw in normalized for kw in ["pleasecomplete", "captcha", "verify", "human"]):
                ...
                if captcha_url:
                    ...
                    has_key = self._has_solver_key(sol_cfg)
                    if sol_cfg.get("enabled", True) and has_key:
                        self.bot.log("SYS", "Attempting BrowserUse auto-solve...")
                        autosolved = await self.bot.web_solver.auto_verify()
                        if autosolved:
                            self.bot._solving_captcha = False
                            self.bot.log("SUCCESS", "BrowserUse solved successfully!")
                            self._show_desktop_notification("BrowserUse solved successfully!")
                        else:
                            self.bot.log("ERROR", "BrowserUse auto-solve failed!")
                            self._show_desktop_notification("BrowserUse failed! Solve manually.")
...
async def setup(bot):
    await bot.add_cog(Security(bot))        self.bot = bot
        cfg = bot.config.get('security', {})
        self.enabled = cfg.get('enabled', True)
        self.notifications_enabled = cfg.get('notifications', {}).get('enabled', True)
        self.notification_title = cfg.get('notifications', {}).get('desktop', {}).get('title', "Neura Security Alert")
