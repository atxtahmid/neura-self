...
with the NoneCap extension installed (CDP mode).
"""
...
class BrowserlessService:
    def __init__(self, bot, api_key, site_key):
        ...
        # CDP endpoint (no /chromium/playwright path) — required for extensions
    ...
    async def solve_full_flow(self, discord_token, retries=3):
        ...
        # Launch options: load NoneCap extension
        ...
        # Query params: token, launch options, and enable Browserless's built-in captcha solver
        params = {
            "token": self.api_key,
            "launch": launch_opts,
            "solveCaptchas": "true",   # Browserless's own CDP-based captcha solver (fallback)
            "timeout": "180000",
        }
        ws_url = f"{self.ws_endpoint}?{urlencode(params)}"
        ...
        for attempt in range(retries):
            ...
            try:
                ...
                async with async_playwright() as p:
                    # Must use connect_over_cdp for extensions
                    browser = await p.chromium.connect_over_cdp(ws_url, timeout=60000)

                    # Extensions are loaded in the DEFAULT context only
                    ...
                    # Inject Authorization header on every Discord/OwO API request
                    ...
                    # Attach a CDP session so we can listen for Browserless captcha events
                    try:
                        cdp = await context.new_cdp_session(page)
                        cdp.on("Browserless.captchaAutoSolved", lambda _: None)
                        cdp.on("Browserless.captchaFound", lambda _: None)
                    except Exception as e:
                        self.bot.log("WARN", f"Browserless: CDP session attach failed (non-fatal): {e}")
    ...
    async def _verify_with_owo(self, token):
        try:
            ...
            return False            "?client_id=408785106942164992"
            "&response_type=code"
            "&redirect_uri=https://owobot.com/api/auth/discord/redirect"
            "&scope=identify guilds"
        )
